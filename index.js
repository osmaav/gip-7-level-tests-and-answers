document.addEventListener('DOMContentLoaded', function () {
        ...
        function processMutationRecord(mutationRecord) {
          ...
          if (needRefresh) {
            checkStylesheetSet(); // Не будем делать обработку слишком часто
            // location.reload();
          }
        }
        ...
        function checkStylesheetSet() {
          ...
          for (var i = 0, l = document.styleSheets.length; i < l; i++) {
            ...
            if (sheet.processed || sheet.cssRules.length === 0) {
              continue;
            }
            sheetCheckResult = checkCSSRuleSet(sheet);
            if (sheetCheckResult.cssRules.length > 0 && sheetCheckResult.selectors.length > 0) {
              Array.prototype.push.apply(allHoverSelectors, sheetCheckResult.selectors);
              Array.prototype.push.apply(allRules, sheetCheckResult.cssRules);
            }
        ...
        // var checkStylesheetSetDebonuced = checkStylesheetSet;
        // .debounce(420);
